namespace ts {
    ...
    export function createPrinter(printerOptions: PrinterOptions = {}, handlers: PrintHandlers = {}): Printer {
        ...
        function beforeEmitNodeWithContext(node: Node, shouldEmitComments: boolean, shouldEmitSourceMaps: boolean) {
            if (shouldEmitComments) emitCommentsBeforeNode(node);
            if (shouldEmitSourceMaps) emitSourceMapsBeforeNode(node);
            beforeEmitNode(node);
        }
        ...
        function afterEmitNodeWithContext(node: Node, shouldEmitComments: boolean, shouldEmitSourceMaps: boolean, savedContainerPos: number, savedContainerEnd: number, savedDeclarationListContainerEnd: number, savedPreserveSourceNewlines: boolean | undefined) {
        ...
        function pipelineEmitWithHint<T extends Node>(emitHint: EmitHint, node: T) {
            onBeforeEmitNode?.(node);
            if (preserveSourceNewlines) {
                const savedPreserveSourceNewlines = preserveSourceNewlines;
                beforeEmitNode(node);
                pipelineEmitWithHintWorker(emitHint, node);
                afterEmitNode(savedPreserveSourceNewlines);
            }
            else {
                pipelineEmitWithHintWorker(emitHint, node);
        ...
        function pipelineEmitWithHintWorker(emitHint: EmitHint, node: Node): void {
            if (emitHint === EmitHint.SourceFile) return emitSourceFile(cast(node, isSourceFile));
            if (emitHint === EmitHint.IdentifierName) return emitIdentifier(cast(node, isIdentifier));
            if (emitHint === EmitHint.JsxAttributeValue) return emitLiteral(cast(node, isStringLiteral), /*jsxAttributeEscape*/ true);
            if (emitHint === EmitHint.MappedTypeParameter) return emitMappedTypeParameter(emitHint, cast(node, isTypeParameterDeclaration));
            if (emitHint === EmitHint.EmbeddedStatement) return emitEmbeddedEmptyStatement(emitHint, cast(node, isEmptyStatement));
            if (emitHint === EmitHint.Unspecified) {
                switch (node.kind) {
                    // Pseudo-literals
                    ...
                    // Expressions (but not really an expression)
                ...
                if (isExpression(node)) {
                    emitHint = EmitHint.Expression;
                    if (substituteNode !== noEmitSubstitution) {
                        const substitute = substituteNode(emitHint, node) || node;
                        if (substitute !== node) {
                            node = substitute;
                            if (currentParenthesizerRule) {
            ...
            if (emitHint === EmitHint.Expression) {
                switch (node.kind) {
                    // Literals
        ...
        function emitMappedTypeParameter(_hint: EmitHint, node: TypeParameterDeclaration): void {
            emit(node.name);
            writeSpace();
            writeKeyword("in");
            writeSpace();
        ...
        function createEmitBinaryExpression() {
            ...
            function onEnter(node: BinaryExpression, state: WorkArea | undefined) {
                if (state) {
                    ...
                    onBeforeEmitNode?.(node);
                    beforeEmitNodeWithContext(node, emitComments, emitSourceMaps);
                }
                else {
                    state = {
                        stackIndex: 0,
            ...
            function onExit(node: BinaryExpression, state: WorkArea) {
                ...
                if (state.stackIndex > 0) {
                    ...
                    afterEmitNodeWithContext(node, shouldEmitComments, shouldEmitSourceMaps, savedContainerPos, savedContainerEnd, savedDeclarationListContainerEnd, savedPreserveSourceNewlines);
                    onAfterEmitNode?.(node);
                    state.stackIndex--;
                }
            }
        ...
        function emitEmbeddedEmptyStatement(_hint: EmitHint, _node: EmptyStatement) {
        ...
        function emitJsxAttributeValue(node: StringLiteral | JsxExpression): void {
            if (isStringLiteral(node)) {
                pipelineEmit(EmitHint.JsxAttributeValue, node);
            }
            else {
